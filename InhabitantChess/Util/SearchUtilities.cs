    ...
    internal static class SearchUtilities
    {
        ...
        public static GameObject Find(string path, bool useResources = true, bool warn = true)
        {
            ...
            if (go == null)
            {
                ...
                if (root == null)
                {
                    if (warn) InhabitantChess.Instance.ModHelper.Console.WriteLine($"Couldn't find root object in path {path}", OWML.Common.MessageType.Warning);
                    return null;
                }
                ...
                {
                    var name = names.Last();
                    if (warn) InhabitantChess.Instance.ModHelper.Console.WriteLine($"Couldn't find object in path {path}, will look for potential matches for name {name}", OWML.Common.MessageType.Warning);
                    ...
                    if (useResources)
                    {
                        ...
                        if (go == null)
                        {
                            if (warn) InhabitantChess.Instance.ModHelper.Console.WriteLine($"Couldn't find object with name {name}", OWML.Common.MessageType.Warning);
                            return null;
                        }
                    }
                }
